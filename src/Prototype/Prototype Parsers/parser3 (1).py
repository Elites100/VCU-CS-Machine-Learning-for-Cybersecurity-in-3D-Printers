...
def read_gcode_file(input_file_path, output_file_path):
    with open(input_file_path, 'r') as input_file, open(output_file_path, 'w', newline='') as output_file:
        ...
        # Write Header
        header = ['Layer', 'Fan on/off', 'Fan speed', 'Change - Fan speed', 'Temperature', 'Temperature Change', 'Extrusion Mode', 'Feedreate', 'Feedrate Change', 'Layer height', 'Layer height change', 'Unknown Commands', 'Regex', 'Modified']
        ...
        for line in input_file:
            ...
            if not matched:
                ...
                csv_writer.writerow([layer, fan_on, fan_speed, '', temperature, temperature_change, extrusion_mode, current_feedrate, feedrate_change, current_height, layer_height_change, unknown_command_line, regex_number, '0'])
                continue
            ...
            if 'Z' in line:
                ...
                # Calculate layer height change if both previous and current heights are available
                if current_height:
                    if previous_height != '-1' and previous_height != current_height:
                        layer_height_change = float(current_height) - float(previous_height)
                    else:
                        layer_height_change = 0  # Set to 0 for the first layer
                        previous_height = float(current_height)
                else:
                    layer_height_change = None

            else:
                layer_height_change = ''


            # Extract feedrate and calculate feedrate change
            if 'F' in line:
                previous_feedrate = current_feedrate
                current_feedrate = extract_feedrate(line)

                # Calculate feedrate change is previous_feedrate and current_feedrate are available
                if current_feedrate:
                    if previous_feedrate != '-1' and previous_feedrate != current_feedrate:
                        feedrate_change = float(current_feedrate) - float(previous_feedrate)
                    else:
                        feedrate_change = 0  # Set to 0 for the first layer
                        previous_feedrate = float(current_feedrate)
                else:
                    feedrate_change = None
            ...
            if temperature != current_temperature:
                temperature_change = abs(float(temperature) - float(current_temperature))
...
if __name__ == "__main__":
    folder_path = r'C:\Users\mabro\Desktop\Spring 2024\AI\parsers\two_files'
