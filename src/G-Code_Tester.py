...
def read_gcode_file(input_file_path):
    """
    ...
    csv_path = os.path.join(current_dir, 'Preprocessed_GCode.csv')
    
    with open(input_file_path, 'r') as input_file, open(csv_path, 'w', newline='') as output_file:
...
def preprocess_csv(csv_data):
    ...
    csv_data = csv_data.drop(columns=['modified'])

    # Ensure the remaining columns match the expected number of features (13)
    assert csv_data.shape[1] == 13, "Number of features in CSV data does not match the expected input shape"

    # Reshape the data to match the expected input shape of the model
    csv_data = np.expand_dims(csv_data.values, axis=-1)  # Add a new axis for the third dimension

    return csv_data

def predict_modification(model, csv_data):
    ...
    # Assuming the model outputs binary predictions (0 for not modified, 1 for modified)
    return predictions

def main():
    # Load the trained MIL model
    model_path = 'src\Production Model\Model-1'  # Path to your trained model
    model = load_model(model_path)

    # Path to the new G-code file to be predicted
    gcode_file = input("Insert G-Code File:\n")
    
    # Preprocess the new G-code file
    Processed_G_Code = read_gcode_file(gcode_file)
    
    # Read the preprocessed CSV data
    csv_data = pd.read_csv(Processed_G_Code)
    
    # Predict whether the CSV has been modified
    predictions = predict_modification(model, csv_data)
    predictions_prob = np.array(predictions)
    
    # Convert predicted probabilities to percentages
    predicted_percentages = predictions_prob * 100

    # Calculate the average predicted percentage for each class
    average_percentage_class1 = tf.convert_to_tensor(np.mean(predicted_percentages[:, 0]))
    average_percentage_class2 = tf.convert_to_tensor(np.mean(predicted_percentages[:, 1]))

    # Print the average percentages for each class
    print(f"Unmodified Prediction: {average_percentage_class1:.2f}%")
    print(f"Modified Prediciton: {average_percentage_class2:.2f}%")
